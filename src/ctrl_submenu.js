...
import componentMove from "./modal_move.js";
...
function componentLeft(render, { $scroll, getSelectionLength$ }) {
    ...
    // Tastaturkuerzel: F2=Umbenennen, Del=Loeschen, M=Verschieben
    effect(rxjs.fromEvent(window, "keydown").pipe(
        rxjs.filter((e) => {
            if (e.target.tagName === "TEXTAREA") return false;
            if (e.target.tagName === "INPUT" && !e.target.classList.contains("hidden")) return false;
            if (document.querySelector("component-modal")) return false;
            if (!document.querySelector('[data-action="delete"]')) return false;
            return e.key === "F2" || e.key === "Delete" || e.key === "m" || e.key === "M";
        }),
        rxjs.tap((e) => {
            e.preventDefault();
            const $btn = e.key === "F2"
                ? document.querySelector('[data-action="rename"]')
                : e.key === "Delete"
                ? document.querySelector('[data-action="delete"]')
                : document.querySelector('[data-action="move"]');
    ...
    effect(getSelectionLength$.pipe(
        ...
        rxjs.map(() => render(createFragment(`
            ...
            <button data-action="move" title="Verschieben"${toggleDependingOnPermission(currentPath(), "rename")}>
                Verschieben
            </button>
        ...
        rxjs.switchMap(($page) => rxjs.merge(
            ...
            onClick(qs($page, `[data-action="move"]`)).pipe(rxjs.mergeMap(() => {
                const path = expandSelection()[0].path;
                return rxjs.from(componentMove(
                    createModal(modalOpt),
                    path,
                )).pipe(rxjs.mergeMap((newpath) => {
                    clearSelection();
                    clearCache(path);
                    clearCache(newpath);
                    return mv(path, newpath);
        ...
        rxjs.mergeMap(($page) => rxjs.merge(
            ...
            onClick(qs($page, `[data-action="move"]`)).pipe(rxjs.mergeMap(() => {
                ...
                return rxjs.from(componentMove(
                    createModal(modalOpt),
                    firstPath,
                )).pipe(rxjs.mergeMap((newdir) => {
                    // newdir = full path of first file in target, extract the directory
                    const targetDir = newdir.replace(/\/[^\/]*$/, "").replace(/\/?$/, "/");
                    clearSelection();
                    return rxjs.forkJoin(paths.map((path) => {
                        const name = path.replace(/\/$/, "").split("/").pop();
                        const isDir = path.endsWith("/");
                        const newpath = targetDir + name + (isDir ? "/" : "");
                        clearCache(path);
                        clearCache(newpath);
                        return mv(path, newpath);
                    }));
