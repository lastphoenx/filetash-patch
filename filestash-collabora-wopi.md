...
| Collabora extern | — | https://o365.example.app | 443 |

Der `filestash`-Docker-Container läuft auf CT 122. Collabora läuft als **nativer systemd-Dienst** (`coolwsd`) auf VM 102 (`o365.example.app`, Debian 13 Trixie) – nicht mehr als Docker-Container. Die interne Erreichbarkeit von CT 122 zu VM 102 erfolgt über `172.19.0.2:9980`.
...
| Filestash Server | `https://files.example.app` | URL von Filestash aus Sicht des Collabora-Containers |
| Office Server | `http://172.19.0.2:9980` | Collabora intern (Docker-Netzwerk IP) |
| Rewrite Discovery | `https://o365.example.app` | Externe Collabora-URL für den Browser |
...
**Rewrite Discovery** ist nötig weil Collabora in der Discovery-Antwort `http://172.19.0.2:9980` zurückgibt – das ist für den Browser nicht erreichbar. Filestash ersetzt diese URL durch `https://o365.example.app`.

---

## Collabora Konfiguration (coolwsd.xml)

Datei: `/etc/coolwsd/coolwsd.xml` auf dem Collabora-Host (`root@o365.example.app`, VM 102 auf pve2)
...
<alias_groups desc="default mode is 'first'...">
    <group>
        <host allow="true" desc="Nextcloud">https://cloud.example.app:443</host>
    </group>
    <group>
        <host allow="true" desc="Office">https://office.example.app:443</host>
    </group>
    <group>
        <host allow="true" desc="Office2">https://office2.example.app:443</host>
    </group>
    <group>
        <host allow="true" desc="Filestash extern">https://files.example.app</host>
    </group>
    <group>
...
Datei: `/etc/nginx/sites-available/files.example.app.conf`
...
Filestash holt Discovery von http://172.19.0.2:9980/hosting/discovery
        ↓
Filestash rewritet URLs: 172.19.0.2:9980 → https://o365.example.app
        ↓
Browser lädt Collabora-Editor von https://o365.example.app
        ↓
Collabora holt Datei via WOPI: GET https://files.example.app/api/wopi/files/...
        ↓
nginx leitet /api/wopi/ ohne Authentik weiter → Filestash → SMB → pi-nas
        ↓
Collabora zeigt Dokument im Browser-Editor
        ↓
Beim Speichern: Collabora POST https://files.example.app/api/wopi/files/.../contents
        ↓
...
- `files.example.app` muss in `coolwsd.xml` unter `alias_groups` eingetragen sein
